...
static GBitmap *s_res_image_ellipsis;
...
static void send_command(int command) {
  ...
  Tuplet value = TupletCString(command, lock_names_list[s_active_lock_name_index]);
...
static void select_click_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  if (strcmp(text_layer_get_text(lock_name_text_layer), "Open Pebble App to configure") == 0 ||
      strcmp(text_layer_get_text(lock_name_text_layer), "Loading locks...") == 0 ||
      strcmp(text_layer_get_text(lock_name_text_layer), "Pebble disconnected") == 0) {
    return;
  }
...
static void window_load(Window *window) {
  ...
  s_res_image_ellipsis = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_ELLIPSIS);
  ...
  action_bar_layer_set_icon(actionbar_layer, BUTTON_ID_SELECT, s_res_image_ellipsis);
  layer_set_hidden((Layer *)actionbar_layer, true);
  ...
  GEdgeInsets insets = GEdgeInsets(
      PADDING,
      layer_get_bounds(action_bar_layer_get_layer(actionbar_layer)).size.w + PADDING * 3,
      PADDING,
      PADDING);
  GRect bounds = grect_inset(layer_get_bounds(window_get_root_layer(window)), insets);
  int center_y = bounds.size.h / 2;

  // lockitron_lock_bitmap_layer
  lockitron_lock_bitmap_layer = bitmap_layer_create(
      GRect(bounds.origin.x, center_y - 25, bounds.size.w, 50));
  GRect lockitron_lock_frame = layer_get_frame((Layer *)lockitron_lock_bitmap_layer);
  bitmap_layer_set_alignment(lockitron_lock_bitmap_layer, GAlignRight);
  ...
  layer_set_hidden((Layer *)lockitron_lock_bitmap_layer, true);

  // lock_name_text_layer
  lock_name_text_layer = text_layer_create(
      GRect(bounds.origin.x, lockitron_lock_frame.origin.y - PADDING - 48, bounds.size.w, 76));
  text_layer_set_background_color(lock_name_text_layer, GColorClear);
  text_layer_set_text(lock_name_text_layer, "Loading locks...");
  text_layer_set_text_alignment(lock_name_text_layer, GTextAlignmentRight);
  text_layer_set_font(lock_name_text_layer, s_res_gothic_24_bold);
  layer_add_child(window_get_root_layer(window), (Layer *)lock_name_text_layer);
  text_layer_enable_screen_text_flow_and_paging(lock_name_text_layer, PADDING);

  // lockstatus_text_layer
  lock_status_text_layer = text_layer_create(
      GRect(
          bounds.origin.x,
  ...
  text_layer_set_font(lock_status_text_layer, s_res_gothic_28_bold);
...
void in_received_handler(DictionaryIterator *received, void *context) {
  ...
  if (lockName_tuple) {
    ...
    if (s_active_lock_name_index >= numberOfLocks) {
      ...
      layer_set_hidden((Layer *)actionbar_layer, false);
      layer_set_hidden((Layer *)lockitron_lock_bitmap_layer, false);
